...
interface CheckInDropdownProps {
  ...
  onAutoOpenCheckout: () => void;
  setCheckInLabel: (label: string) => void;
  setTimeFrame?: (label: string) => void;
}

type Tab = "Dates" | "Months" | "Flexible";

const CheckInDropdown: React.FC<CheckInDropdownProps> = ({
  selectedDate,
  onDateSelect,
  onAutoOpenCheckout,
  setCheckInLabel,
  ...
  // === Scroll Controls for Flexible Tab ===
  const scrollLeft = () => {
    ...
    if (activeTab === "Flexible") {
      return (
        <div className="flex flex-col items-center mt-2 relative">
          ...
          {/* Scrollable months with buttons */}
          ...
          {(["Dates", "Months", "Flexible"] as Tab[]).map((tab) => (
            <button
              ...
              onClick={(e) => {
                ...
                if (tab === "Dates") setCheckInLabel("Check in");
              }}
