...
const PhotoDetail = () => {
  ...
  const showPrev = () => {
    ...
    for (let i = 0; i < groupedImages.length; i++) {
      ...
      if (prevGlobalIndex < count + len)
        return setLightbox({ sectionIndex: i, imageIndex: prevGlobalIndex - count });
      count += len;
    }
  };
  ...
  const showNext = () => {
    ...
    for (let i = 0; i < groupedImages.length; i++) {
      const len = groupedImages[i].images.length;
      if (nextGlobalIndex < count + len)
        return setLightbox({ sectionIndex: i, imageIndex: nextGlobalIndex - count });
      count += len;
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto mt-50 px-4 sm:px-6 lg:px-8">
      ...
      {/* ======= Vertical Sections (Airbnb-style clean layout) ======= */}
      {groupedImages.map((section, sectionIndex) => (
        <div
          ...
          className="mb-20 md:flex md:gap-8"
        >
          {/* Left: Section Title */}
          <div className="md:w-1/4 md:pt-2">
          ...
          {/* Right: Image Grid */}
          <div className="md:w-3/4 grid grid-cols-2 md:grid-cols-3 gap-3">
            {section.images.map((img, i) => (
              <div
                key={i}
                className={`cursor-pointer overflow-hidden ${
                  i === 0
                    ? "col-span-2 row-span-2 h-80" // Large first image
                    : "h-40" // Smaller images
                }`}
        ...
        {lightbox && (
          ...
          >
            <button
              ...
              className="absolute top-6 right-6 text-white text-lg font-semibold p-2 hover:underline"
            >
              ✕
            </button>
            <button
